...
export async function parseBillOCR(ocrText: string): Promise<BillLineItem[]> {
...
${ocrText}`
  ...
  const result = await generateJSON<BillLineItem[]>(prompt)
...
export async function parseVoiceCommand(transcript: string): Promise<VoiceIntent> {
...
Voice command: ${transcript}`
  ...
  const result = await generateJSON<VoiceIntent>(prompt)
...
Return JSON only, no explanation:
{
  ...
  return generateJSON<ProductMetadata>(prompt)
}
...
export async function businessChat(
  ...
  if (!apiKey) return 'AI assistant is not configured. Please add GEMINI_API_KEY to the API server.'
